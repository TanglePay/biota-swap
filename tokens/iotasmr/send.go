...
func (ist *IotaSmrToken) CheckUnWrapTx(txid []byte, to, symbol string, amount *big.Int) error {
	return nil
}

func (ist *IotaSmrToken) CreateUnWrapTxData(ed25519Id string, amount *big.Int) ([]byte, []byte, error) {
	...
	if ist.tokenID.Empty() {
		return ist.getUnSignedTxDataBasic(addr, amount)
	}
	return ist.getUnSignedTxDataNativeToken(addr, amount)
}
...
func (ist *IotaSmrToken) getUnSignedTxDataBasic(toAddr iotago.Address, amount *big.Int) ([]byte, []byte, error) {
...
func (ist *IotaSmrToken) getUnSignedTxDataNativeToken(toAddr iotago.Address, amount *big.Int) ([]byte, []byte, error) {
	info, err := ist.nodeAPI.Info(context.Background())
	if err != nil {
		return nil, nil, fmt.Errorf("get iotasmr node info error. %v", err)
	}
